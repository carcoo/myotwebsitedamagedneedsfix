	...
	if (!empty($_POST['buy']) && $_SESSION['shop_session'] == $_POST['session']) {
		...
		if ($player_points >= $buy['points']) {
			...
			if ($buy['type'] == 2) {
				...
				mysql_insert("INSERT INTO `znote_shop_orders` (`account_id`, `type`, `itemid`, `count`, `time`) VALUES ('$cid', '". $buy['type'] ."', '". $buy['itemid'] ."', '". $buy['count'] ."', '$time')");
				echo '<font color="green" size="4">Your order is ready to be delivered. Write this command in-game to get it if it took more than 5 minutes: [!shop].<br>Make sure you are in depot and can carry it before executing the command!</font>';
			}
...
/* 	if ($config['shop_auction']['characterAuction']) {
		?>
		<p>Interested in buying items from players? View the <a href="auctionChar.php">auction page!</a></p>
		<?php
	} */
} else {
...
<div id="categoryNavigator">
	<?php if (!empty($category_items)): ?><a href="shop.php#cat_itemids"><b>ITEMS</b></a><?php endif; ?>
	<?php if (!empty($category_mounts)): ?><a href="shop.php#cat_mounts"><b>MOUNTS</b></a><?php endif; ?>
	<?php if (!empty($category_mounts)): ?><a href="shop.php#cat_misc"><b>MISC</b></a><?php endif; ?>
</div>
</center>
		...
		<?php foreach ($category_items as $key => $offers): ?>
			<tr class="special">
				...
				<?php if ($config['shop']['showImage']):?>
					<td><img src="layout/items/<?php echo $offers['itemid']; ?>.<?php echo $config['shop']['imageType']; ?>" alt="img"></td>
				<?php endif; ?>
    ...
    $(document).ready(function(){
        $(".needconfirmation").each(function(e){
            $(this).click(function(e){
				...
				var r = confirm("Do you really want to purchase this item for "+itemcost+" points?")
				if(r == false){
					e.preventDefault();
				}			
            });
