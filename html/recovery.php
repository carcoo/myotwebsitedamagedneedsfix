...
if ($config['mailserver']['accountRecovery']) {
	...
	if (!empty($_POST)) {
		...
		if ($config['use_captcha']) {
			$captcha = (isset($_POST['g-recaptcha-response'])) ? $_POST['g-recaptcha-response'] : true;
			if(!$captcha) {
				$status = true;
			} else {
	...
	<p>The admin have disabled automatic account recovery for your accounts safety msg any admin in game to help you getting it back.</p>
	<?php
}
