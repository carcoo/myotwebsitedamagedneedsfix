			...
			<div class="container_right">
						...
						<div class="rise-up-content">
								...
								<div class="login"></div>
										<form action="login.php" method="post" style="margin-bottom: 0;">
											<input type="text" name="username" value="Account name" class="inputtext" onfocus="this.value=''" onblur="if(this.value=='') { this.value='Account number'};">
											...
											<center style="font-size: 12px;">
												Lost <a href="recovery.php?mode=username">ACC Name</a> or <a href="recovery.php?mode=password">Password</a>?
											</center>
										</form>
								<?php }else{ ?>
								<div class="acc_menu">
				...
				<div class="right_box">
					<div class="corner_lt"></div><div class="corner_rt"></div><div class="corner_lb"></div><div class="corner_rb"></div>
					<div class="title"><img src="layout/img/exp.gif"><span style="background-image: url(layout/widget_texts/topplayers.png);"></span></div>
					<div class="content">
						<div class="rise-up-content">
						<ul class="toplvl">
						<?php
							function coloured_value($valuein)
							{
								...
								@$value = $value2.$value;
								if($valuein > 0)
									return '<b><font color="#D3D3D3">'.$value.'</font></b>';
								elseif($valuein < 0)
									return '<font color="red">'.$value.'</font>';
								else
									return $value;
							}
							$cache = new Cache('engine/cache/top10');
							if ($cache->hasExpired()) {
								$znotePlayers = mysql_select_multi('SELECT * FROM players WHERE group_id < 2 ORDER BY  level DESC LIMIT 5;');
							...
							if($znotePlayers){
								foreach($znotePlayers as $player)
								{
									...
									echo '<span style="float: right;"><strong>'.coloured_value($player['level']).'</strong></span></li>';
								}
							}
							?>
							</ul>
				...
				<div class="right_box">
					...
					<div class="content">
						...
						<?php
							...
							if ($cache->hasExpired()) {
								$znotePlayers = mysql_select_multi('SELECT * FROM live_casts WHERE znote = 1 ORDER BY spectators DESC LIMIT 5;');
								$cache->setContent($znotePlayers);
								$cache->save();
							} else {
								$znotePlayers = $cache->load();
							...
							if($znotePlayers){
								foreach($znotePlayers as $live_casts)
								{
									$nam = $live_casts['cast_name'];
									if (strlen($nam) > 15)
									{$nam = substr($nam, 0, 12) . '...';}
									echo '<li style="margin: 6px 0;"><div style="position:relative; left:-48px; top:-48px;"><div style="background-image: url(layout/outfitter/outfit.php?id='.$player['looktype'].'&head='.$player['lookhead'].'&body='.$player['lookbody'].'&legs='.$player['looklegs'].'&feet='.$player['lookfeet'].');width:64px;height:64px;position:absolute;background-repeat:no-repeat;background-position:right bottom;"></div></div>
									<a style="margin-left: 19px;" href="characterprofile.php?name=' .$live_casts['cast_name']. '">' .$nam. '</a>';
									
									echo '<span style="float: right;">'.$live_casts['spectators'].'</span></li>';
								}
			...
			<div class="footer_cnt">
				<center>Copyright &copy; <?php echo date('Y', time()); ?> <strong>Carcooizx.zapto.org</strong>. All rights reserved.<br><a target="_blank" href="http://carcooizx.org/characterprofile.php?name=Admin%20Carcoo" style="color: #3d4654;font-size: 11px;">by Carcoo</a></center>
			</div>
	</body>
</html>
